...
import {deleteNews, editNews, getNews, manyDeleteNews} from '@/services/news';
...
const News = () => {
    ...
    useEffect(() => {
        const fetchSortedAndPaginatedNews = async () => {
            try {
                ...
                if (response.data) {
                    setNewsList(response.data.data); // Updated news list
                    setTotalRecords(response.data.total); // Retrieve the total record count from the API
                } else {
    ...
    const handleDeleteNews = async (id: number) => {
        try {
            await deleteNews(id);
            // Update the news list after successfully deleting a news article
            const response = await getNews(sortStatus.columnAccessor, page, pageSize);
            succesToastMessage(`News deleted successfully`, 1500);
            setNewsList(response.data.data);
            setTotalRecords(response.data.total);
        } catch (error) {
            console.error('An error occurred while deleting the news:', error);
            errorToastMessage(`News could not be deleted`, 1500);
        }
    };
    ...
    const refreshNewsList = async () => {
        try {
            ...
            console.error('An error occurred while fetching the news again:', error);
        }
    };

    const selectNewsForEdit = (newsItem:any) => {
        setInfo(newsItem); // Store the information of the selected news
        setOpen1(true); // Open the Modal
    };

    const handleNewNews = () => {
        setInfo({ ...defaultParams }); // Reset the info state with defaultParams
    ...
    const handleManyDelete = async () => {
        try {
            ...
            succesToastMessage(`Selected languages were successfully deleted`, 1500);
            await refreshNewsList();
        } catch (error) {
            console.error('An error occurred while deleting languages:', error);
            errorToastMessage(`Languages could not be deleted.`, 1500);
        }
    };
    ...
    const handleCheckChange = async (newsId: never, newIsActive: boolean) => {
        try {
            ...
            if (response.data) {
                succesToastMessage(`News status updated successfully`, 1500);
                refreshNewsList();
            }
        } catch (error) {
            console.error('Error updating news status:', error);
            errorToastMessage(`An error occurred while updating the news status`, 1500);
        }
    };
