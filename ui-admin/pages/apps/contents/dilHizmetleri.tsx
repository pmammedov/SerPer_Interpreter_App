...
import {deleteLanguages, getLanguages, manyDeleteLanguages, editLanguages} from "@/services/languages";
import 'react-toastify/dist/ReactToastify.css';
import {errorToastMessage, succesToastMessage} from "@/components/toastify";
import DilHizmetleriModal from "@/pages/apps/contents/components/dilHizmetleriModal";

const DilHizmetleri = () => {
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('Hizmet Verdiğimiz Diller'));
    });
    const defaultParams = {
        id: null,
        name_tr: '',
    ...
    useEffect(() => {
        const fetchSortedAndPaginatedNews = async () => {
            try {
                ...
                const response = await getLanguages(sortOrder, page, pageSize, search);
                if (response.data) {
                    setLanguagesList(response.data.data); // Güncellenmiş dil listesi
    ...
    const handleDeleteLanguages = async (id: number) => {
        try {
            await deleteLanguages(id);
            // Diller başarıyla silindikten sonra diller listesini güncelle
            const response = await getLanguages(sortStatus.columnAccessor, page, pageSize);
            succesToastMessage(`Dil başarı ile silindi.`, 1500);
            setLanguagesList(response.data.data);
            setTotalRecords(response.data.total);
        } catch (error) {
            console.error('Dil silinirken hata oluştu:', error);
            errorToastMessage(`Dil silinemedi.`, 1500);
        }
    };

    const refreshLanguagesList = async () => {
        try {
            const response = await getLanguages(sortStatus.columnAccessor, page, pageSize);
            setLanguagesList(response.data.data);
            setTotalRecords(response.data.total);
        } catch (error) {
            console.error('Diller yeniden çekilirken hata oluştu:', error);
        }
    };

    const selectLanguagesForEdit = (newsItem:any) => {
    ...
    const handleManyDelete = async () => {
        try {
            ...
            await manyDeleteLanguages({ ids: idsToDelete });
            succesToastMessage(`Seçilen diller başarı ile silindi.`, 1500);
            await refreshLanguagesList();
        } catch (error) {
    ...
    const handleCheckChange = async (languagesId: never, newIsActive: boolean) => {
        try {
            const currentUser = languagesList.find((news: { id: any }) => news.id === languagesId);
            if (!currentUser) {
                throw new Error('Dil hizmeti bulunamadı');
            }

            const response = await editLanguages(languagesId, {
                is_active: newIsActive,
            });

            if (response.data) {
                succesToastMessage(`Dil hizmeti durumu başarılı ile güncellendi.`, 1500);
                refreshLanguagesList();
            }
        } catch (error) {
            console.error('Dil hizmeti durumu güncellenirken hata:', error);
            errorToastMessage(`Dil hizmeti durumu güncellenirken hata oluştu.`, 1500);
        }
    };
    ...
    return (
        <div className="panel border-white-light px-0 dark:border-[#1b2e4b]">
            <div className="invoice-table">
                <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={handleNewLanguages} className="btn btn-primary gap-2">
                    ...
                    <DataTable
                        ...
                        columns={[
                            ...
                            },
                            {
                                title: 'Resim',
                                accessor: 'image',
                                render: ({ image }) => (image ? <img src={image} alt="Dil Resmi" style={{ width: '50px', height: '50px' }} /> : 'Resim Yok'),
                            },
                            {
                                title: 'Dil Adı (Türkçe)',
                                accessor: 'name_tr',
                                render: ({ name_tr }) => (
                                    <div className="flex items-center font-semibold">
                                        <div>{name_tr}</div>
                            ...
                            },
                            {
                                ...
                                render: ({ id, name_tr, name_en, name_ar, image, order,is_active }) => (
                                    <div className="mx-auto flex w-max items-center gap-4">
                                        <button onClick={() => selectLanguagesForEdit({ id, name_tr, name_en, name_ar, image, order,is_active })} className="flex hover:text-info">
                                        ...
                                        <button type="button" className="flex hover:text-danger" onClick={() => handleDeleteLanguages(id)}>
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5">
                                                <path d="M20.5001 6H3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"></path>
                                                <path
                                                    d="M18.8334 8.5L18.3735 15.3991C18.1965 18.054 18.108 19.3815 17.243 20.1907C16.378 21 15.0476 21 12.3868 21H11.6134C8.9526 21 7.6222 21 6.75719 20.1907C5.89218 19.3815 5.80368 18.054 5.62669 15.3991L5.16675 8.5"
            ...
            <DilHizmetleriModal
                ...
                info={info}
                setInfo={setInfo}
                refreshLanguagesList={refreshLanguagesList}
