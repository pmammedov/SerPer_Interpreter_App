...
import {getPortfolio, deletePortfolio, manyDeletePortfolio, editPortfolio} from '@/services/portfolio';
import PortfoyModal from './components/portfoyModal';
...
const Portfoy = () => {
    ...
    useEffect(() => {
        const fetchSortedAndPaginatedPortf = async () => {
            try {
                const sortOrder: string = sortStatus.direction === 'asc' ? sortStatus.columnAccessor : `-${sortStatus.columnAccessor}`;
                const response = await getPortfolio(sortOrder, page, pageSize, search);
                // console.log(response);
                if (response.data) {
                    setItems(response.data.data); // Güncellenmiş Portf listesi
                    setTotalRecords(response.data.total); // Toplam kayıt sayısını API'dan al
                } else {
                    console.error('Beklenen veri yapısı alınamadı');
                }
            } catch (error) {
                console.error('Portföyler alınırken hata oluştu:', error);
            }
        };
        // console.log('search', search);
        fetchSortedAndPaginatedPortf();
    ...
    const handleDeletePortfolio = async (id: number) => {
        try {
            ...
            // Portföy başarıyla silindikten sonra haber listesini güncelle
            const response = await getPortfolio(sortStatus.columnAccessor, page, pageSize);
            setItems(response.data.data);
            setTotalRecords(response.data.total);
        } catch (error) {
            console.error('Portfoy silinirken hata oluştu:', error);
        }
    };
    const refreshPortList = async () => {
        try {
            ...
            console.error('Portfoy yeniden çekilirken hata oluştu:', error);
        }
    };
    const selectPortForEdit = (portItem: any) => {
        setInfo(portItem); // Seçilen haberin bilgilerini saklayın
        setOpen1(true); // Modalı açın
    };
    const handlePortNews = () => {
    ...
    const handleManyDelete = async () => {
        try {
            ...
            await manyDeletePortfolio({ ids: idsToDelete });
    ...
    const handleCheckChange = async (portfolioId: never, newIsActive: boolean) => {
        try {
            const currentUser = items.find((portfolio: { id: any }) => portfolio.id === portfolioId);
            if (!currentUser) {
                throw new Error('Portfolio bulunamadı');
            }

            const response = await editPortfolio(portfolioId, {
                is_active: newIsActive,
            });

            if (response.data) {
                succesToastMessage(`Portfolio durumu başarılı ile güncellendi.`, 1500);
                refreshPortList();
            }
        } catch (error) {
            console.error('Portfolio durumu güncellenirken hata:', error);
            errorToastMessage(`Portfolio durumu güncellenirken hata oluştu.`, 1500);
        }
    };
    ...
    return (
        <div className="panel border-white-light px-0 dark:border-[#1b2e4b]">
            <div className="invoice-table">
                <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={handlePortNews} className="btn btn-primary gap-2">
                            ...
                            },
                            {
                                ...
                                render: ({ content_tr }) => (
                                    <div className="flex items-center font-semibold">
                                        ...
                                        >
                                            <div>{content_tr?.length > 25 ? `${content_tr.slice(0, 25)}...` : content_tr}</div>
                                        </Tooltip>
                                    </div>
                                ),
                            },
                            ...
                            },
                            {
                                ...
                                render: ({ content_en }) => (
                                    <div className="flex items-center font-semibold">
                                        ...
                                        >
                                            <div>{content_en?.length > 25 ? `${content_en.slice(0, 25)}...` : content_en}</div>
                                        </Tooltip>
                                    </div>
                                ),
                            },
                            ...
                            },
                            {
                                ...
                                render: ({ content_ar }) => (
                                    <div className="flex items-center font-semibold">
                                        ...
                                        >
                                            <div>{content_ar?.length > 25 ? `${content_ar.slice(0, 25)}...` : content_ar}</div>
                                        </Tooltip>
                                    </div>
                                ),
                            },
                            ...
                            },
                            {
                                ...
                                render: ({ id, title_tr, content_tr, title_en, content_en, title_ar, content_ar, image, is_active }) => (
                                    <div className="mx-auto flex w-max items-center gap-4">
                                        <button
                                            onClick={() => selectPortForEdit({ id, title_tr, content_tr, title_en, content_en, title_ar, content_ar, image, is_active })}
                                        ...
                                        <button type="button" className="flex hover:text-danger" onClick={() => handleDeletePortfolio(id)}>
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5">
                                                <path d="M20.5001 6H3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"></path>
                                                <path
                                                    d="M18.8334 8.5L18.3735 15.3991C18.1965 18.054 18.108 19.3815 17.243 20.1907C16.378 21 15.0476 21 12.3868 21H11.6134C8.9526 21 7.6222 21 6.75719 20.1907C5.89218 19.3815 5.80368 18.054 5.62669 15.3991L5.16675 8.5"
            ...
            <PortfoyModal info={info} setInfo={setInfo} open={open} setOpen={setOpen} open1={open1} setOpen1={setOpen1} refreshPortList={refreshPortList} defaultParams={defaultParams} />
        </div>
    );
};
